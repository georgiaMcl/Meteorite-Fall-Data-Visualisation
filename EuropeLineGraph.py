...
# Ensuring we only get Europe related data
continent_data = continent_data[continent_data["continent"] == "Europe"] 
population_data = population_data[population_data["Entity"] == "Europe"] 
...
# Create a new DataFrame with all years from years_to_include
...
ax.set_ylabel("Human Population of Europe (millions)", color = "darkseagreen", fontsize = 14)

# creating right y-axis
ax2 = ax.twinx()
ax2.plot(x_coord,y_coord_meteorite , color="coral",marker="o")
ax2.set_ylabel("Number of Found Meteorites With Fall Observations in Europe",color="coral",fontsize=14)
fig.suptitle('Europe - Human Population With Number of Observed Falling Meteorites That Were Later Found (from 900 CE to 2000 CE over 50 year increments)', fontsize = 14)
plt.show()
