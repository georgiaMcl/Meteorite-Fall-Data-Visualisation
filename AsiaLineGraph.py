...
# Ensuring we only get Asia related data
continent_data = continent_data[continent_data["continent"] == "Asia"] 
population_data = population_data[population_data["Entity"] == "Asia"] 
...
ax.set_ylabel("Human Population of Asia (millions)", color = "darkseagreen", fontsize = 14)

# creating right y-axis
ax2 = ax.twinx()
ax2.plot(x_coord,y_coord_meteorite , color="coral",marker="o")
ax2.set_ylabel("Number of Found Meteorites With Fall Observations in Asia",color="coral",fontsize=14)
fig.suptitle('Asia - Human Population With Number of Observed Falling Meteorites That Were Later Found (from 900 CE to 2000 CE over 50 year increments)', fontsize = 14)
plt.show()
