...
# Ensuring we only get South America related data
continent_data = continent_data[continent_data["continent"] == "South America"] 
population_data = population_data[population_data["Entity"] == "South America"] 
...
ax.set_ylabel("Human Population of South America (millions)", color = "darkseagreen", fontsize = 14)

# creating right y-axis
ax2 = ax.twinx()
ax2.plot(x_coord,y_coord_meteorite , color="coral",marker="o")
ax2.set_ylabel("Number of Found Meteorites With Fall Observations in South America",color="coral",fontsize=14)
fig.suptitle('South America - Human Population With Number of Observed Falling Meteorites That Were Later Found (from 900 CE to 2000 CE over 50 year increments)', fontsize = 14)
plt.show()
