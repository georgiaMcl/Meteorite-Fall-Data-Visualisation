...
# Ensuring we only get Oceania related data
continent_data = continent_data[continent_data["continent"] == "Oceania"] 
population_data = population_data[population_data["Entity"] == "Oceania"] 
...
ax.set_ylabel("Human Population of Oceania (millions)", color = "darkseagreen", fontsize = 14)

# creating right y-axis
ax2 = ax.twinx()
ax2.plot(x_coord,y_coord_meteorite , color="coral",marker="o")
ax2.set_ylabel("Number of Found Meteorites With Fall Observations in Oceania",color="coral",fontsize=14)
fig.suptitle('Oceania - Human Population With Number of Observed Falling Meteorites That Were Later Found (from 900 CE to 2000 CE over 50 year increments)', fontsize = 14)
plt.show()
